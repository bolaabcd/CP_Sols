...
int main() { _
	while(1) {
		...
		vector<pair<vector<pair<int,int>>,int>> teams(t,{vector<pair<int,int>>(p),-1}); // -1
		...
		for(int i = 0; i < t; i++) {
			...
			for(int j = 0; j < p; j++) {
				...
				assert(cin.peek() == '/');
				cin.get();
				// Note that this breaks specification, but there is one test case with S=0 -_-
				// assert(cin.peek() != '0');
				if(cin.peek() == '-'){
					cin.get();
					teams[i].first[j].second = -1; // -1
				}
				else{
					teamnosps[i]++;
		...
		for(int k = 1; k < t; k++) {
			...
			for(int j = 0; j < p; j++) {
				...
				if(teams[k].first[j].second >= 0) {
					sumTa_Tb += teams[k].first[j].second;
					sumFb_Fa -= teams[k].first[j].first;
				}
				if( teams[k-1].first[j].second >= 0) {
					sumTa_Tb -= teams[k-1].first[j].second;
